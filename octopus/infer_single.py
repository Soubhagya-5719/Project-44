...
def main(weights, name, segm_dir, pose_dir, out_dir, opt_pose_steps, opt_shape_steps):
    ...
    write_mesh('{}/{}_tposed.obj'.format(out_dir, name), pred['vertices_tposed'], pred['faces'])
    json.dump(pred['betas'].tolist(), codecs.open('{}/{}.json'.format(out_dir, name), 'w', encoding='utf-8'), separators=(',', ':'), sort_keys=True, indent=4)    
